...
- [`apps/knx_source_emulator/`](apps/knx_source_emulator/) — локальный KNX-like source emulator для synthetic Config Registry/edge runtime сценариев
...
- `Edge Telemetry Agent`: `apps/edge_telemetry_agent`, локальный southbound emulator `apps/knx_source_emulator`, edge profiles в `environments/` и KNX/parser tooling.
...
- `uv run --package edge-telemetry-agent edge-telemetry-agent run --help`
- `uv run --package knx-source-emulator knx-source-emulator plan --dry-run`
- `uv run --package knx-source-emulator knx-source-emulator start --help`
